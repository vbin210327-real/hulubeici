...
private struct ProfileCenterView: View {
    ...
    @State private var showingHelpAndFeedback = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingHelpAndFeedback) {
            HelpAndFeedbackView()
                .presentationDetents([.medium, .large])
        }
        ...
        .fullScreenCover(isPresented: $showingProfileEditor) {
            ProfileEditView(
                avatarImageData: $userProfile.avatarImageData,
        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .center) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    // Membership badge and expiry date
                    if purchaseStore.subscriptionTier != .none || purchaseStore.hasExpired {
                        VStack(alignment: .leading, spacing: 4) {
                            membershipBadge

                            if let expiryDate = purchaseStore.subscriptionExpirationDate {
                                if purchaseStore.hasExpired {
                                    Text("已于 \(formattedExpiryDate(expiryDate)) 过期")
                                        .font(.system(size: 11))
                                        .foregroundColor(.red)
                                } else {
    ...
    private var membershipBadge: some View {
        ...
        if purchaseStore.hasExpired {
            // Show expired status with previous tier
            let previousTierName = purchaseStore.previousSubscriptionTier == .plus ? "Plus" : "Pro"
            badgeText = "\(previousTierName)会员已过期"
        ...
        } else if purchaseStore.subscriptionTier == .plus {
            badgeText = "Plus会员"
            gradient = LinearGradient(
                gradient: Gradient(colors: [Color(red: 0.4, green: 0.5, blue: 0.95), Color(red: 0.5, green: 0.3, blue: 0.85)]),
                startPoint: .leading,
                endPoint: .trailing
            )
            shadowColor = Color(red: 0.4, green: 0.5, blue: 0.95).opacity(0.4)
        } else {
            badgeText = "Pro会员"
            gradient = LinearGradient(
                gradient: Gradient(colors: [Color(red: 0.95, green: 0.7, blue: 0.3), Color(red: 0.95, green: 0.5, blue: 0.2)]),
                startPoint: .leading,
                endPoint: .trailing
            )
            shadowColor = Color(red: 0.95, green: 0.6, blue: 0.25).opacity(0.4)
        }
    ...
    private var recentActivityCard: some View {
        if let recent = recentSection {
            ...
            Button {
                handleRecentSectionTapped(recent.id)
            } label: {
                ProfileInfoCard(
                    title: "最近学习",
    ...
    private func handleRecentSectionTapped(_ sectionId: UUID) {
        if purchaseStore.isPremium {
            navigationPath.append(sectionId)
        } else {
            showingPaywall = true
        }
    }
...
struct ContentView: View {
    ...
    init() {
        // Use a fixed local user ID for iCloud-only mode
            ...
            tabContent
                ...
                .task {
                    await CloudKitSyncService.shared.initialPull(
                        ...
                        visibilityStore: hideState,
                        userProfile: userProfile
                    )
                    CloudKitSyncService.shared.bind(
                        bookStore: bookStore,
                        progressStore: progressStore,
                        dailyProgressStore: dailyProgressStore,
                        visibilityStore: hideState,
        ...
        .safeAreaInset(edge: .bottom) {
            if isRootView {
                BottomTabBar(
                    ...
                    onAddSection: { handleAddSectionTapped() },
                    showAddButton: true
                )
            } else {
                EmptyView()
    ...
    private var tabContent: some View {
        ZStack {
            ...
            if selectedTab == .profile {
                ProfileCenterView(userProfile: userProfile, navigationPath: $navigationPath)
    ...
    private func handleAddSectionTapped() {
        if !purchaseStore.isPremium && customSectionCount() >= AppEntitlements.freeCustomWordbookLimit {
            showingPaywall = true
        } else {
    ...
    private func handleSectionTapped(_ sectionId: UUID) {
        if purchaseStore.isPremium {
            navigationPath.append(sectionId)
        } else {
            showingPaywall = true
    ...
    private func customSectionCount() -> Int {
        let bundledIDs = Set(BundledWordBookLoader.bundledSectionMetadata.map { $0.0 })
        return bookStore.sections.filter { !bundledIDs.contains($0.id) }.count
    }
            ...
            VStack(spacing: 24) {
                ...
                LazyVStack(spacing: 24) {
                    ForEach(bookStore.sections) { section in
                        ...
                        Button {
                            handleSectionTapped(section.id)
                        } label: {
                            SectionCardView(
                                section: section,
...
struct WordSection: Identifiable, Codable {
    ...
    private enum CodingKeys: String, CodingKey { case id, title, subtitle, words, targetPasses }
...
final class SectionProgressStore: ObservableObject {
    struct ProgressState: Codable, Equatable {
        ...
        mutating func stampStudy() { lastStudiedAt = Date() }
    }

    @Published private var progressStates: [UUID: ProgressState] = [:] {
        didSet { persist(); syncToBackend() }
    }
    ...
    func progress(for sectionID: UUID) -> ProgressState { progressStates[sectionID] ?? ProgressState() }
    func completedPages(for sectionID: UUID) -> Int { max(0, min(progress(for: sectionID).completedPages, Int.max)) }
    func completedPasses(for sectionID: UUID) -> Int { max(0, progress(for: sectionID).completedPasses) }
    ...
    func nextPageIndex(for sectionID: UUID, totalPages: Int, targetPasses: Int) -> Int {
        ...
        if state.completedPasses >= target { return max(totalPages - 1, 0) }
    ...
    func markPageCompleted(sectionID: UUID, totalPages: Int, pageIndex: Int, targetPasses: Int) -> ProgressState {
        ...
        let target = max(1, targetPasses)
        if state.completedPasses >= target && state.completedPages >= totalPages { return state }
        let nextPage = pageIndex + 1
        if nextPage >= totalPages {
            if state.completedPasses + 1 >= target { state.completedPasses = min(state.completedPasses + 1, target); state.completedPages = totalPages }
            else { state.completedPasses += 1; state.completedPages = 0 }
        } else { state.completedPages = max(state.completedPages, nextPage) }
        progressStates[sectionID] = state
        return state
    }

    func recordStudyEvent(for sectionID: UUID) { var s = progress(for: sectionID); s.stampStudy(); progressStates[sectionID] = s }
    fileprivate func clampProgress(for sectionID: UUID, totalPages: Int, targetPasses: Int) {
        var s = progress(for: sectionID); let target = max(1, targetPasses)
        s.completedPasses = min(s.completedPasses, target)
        if totalPages <= 0 { s.completedPages = 0 } else { s.completedPages = min(s.completedPages, totalPages); if s.completedPasses >= target { s.completedPages = totalPages } }
        progressStates[sectionID] = s
    }

    private func persist() {
        guard let data = try? JSONEncoder().encode(progressStates) else { return }
        defaults.set(data, forKey: defaultsKey)
    }

    private func loadInitialState() {
        if let data = defaults.data(forKey: defaultsKey) { _ = decodeProgress(from: data) }
        if defaults.data(forKey: defaultsKey) == nil, let legacy = defaults.data(forKey: Self.legacyDefaultsKey) { if decodeProgress(from: legacy) { persist(); defaults.removeObject(forKey: Self.legacyDefaultsKey) } }
    }

    @discardableResult
    private func decodeProgress(from data: Data) -> Bool {
        let decoder = JSONDecoder()
        if let decoded = try? decoder.decode([UUID: ProgressState].self, from: data) { progressStates = decoded; return true }
        if let legacy = try? decoder.decode([UUID: Int].self, from: data) { progressStates = legacy.reduce(into: [:]) { $0[$1.key] = ProgressState(completedPages: $1.value, completedPasses: 0) }; return true }
        return false
    }

    private func syncToBackend() { }

    // MARK: - Public helpers used by iCloud sync / UI
    ...
    func resetAllProgress() {
        if !progressStates.isEmpty { progressStates.removeAll() }
    }
}

// MARK: - Daily Progress Store
...
extension DailyProgressStore {
    func replaceAll(_ newRecords: [String: Int]) {
        isRestoring = true
        records = newRecords
        isRestoring = false
    }
...
final class UserProfileStore: ObservableObject {
    ...
    /// Load profile from backend (removed). iCloud is authoritative now.
    func loadFromBackend() async { }

    /// Sync profile to backend (removed)
    private func syncToBackend() { }
}
...
extension WordVisibilityStore {
    func replaceAll(hiddenWordIDs: [UUID: Set<UUID>], hiddenMeaningIDs: [UUID: Set<UUID>]) {
        isRestoring = true
        snapshot = Snapshot(hiddenWordIDs: hiddenWordIDs, hiddenMeaningIDs: hiddenMeaningIDs)
        isRestoring = false
    }
